...
const UnorderedList = (param1: any, ...params: any) => ul({
    className: "list-disc pl-6 mb-6 space-y-2",
...param1
}, ...params);

const OrderedList = (param1: any, ...params: any) => ol({
    className: "list-decimal pl-6 mb-6 space-y-2",
...param1
}, ...params);

const ListItem = (param1: any, ...params: any) => li({
    className: "leading-7",
...param1
}, ...params);
...
function parseMarkdownToElements(markdown: string): any[] {
    ...
    let currentList: { type: "ul" | "ol", items: string[][], indent: number } | null = null;
    ...
    function flushList() {
        ...
        const listItems = currentList.items.map(itemLines => {
            const itemChildren: (string | any)[] = [];
            itemLines.forEach((line, idx) => {
                const inlineParts = parseInline(line);
                itemChildren.push(...inlineParts);
                if (idx < itemLines.length - 1) {
                    itemChildren.push(Br({}));
                }
            });
            return ListItem({}, ...itemChildren);
        });
        if (currentList.type === "ul") {
            elements.push(UnorderedList({}, ...listItems));
        } else {
            elements.push(OrderedList({}, ...listItems));
        }
        currentList = null;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (listItemMatch) {
            ...
            if (!currentList || currentList.type !== listType || currentList.indent !== indent) {
                flushList();
                currentList = { type: listType, items: [], indent };
            }
        ...
        if (currentList && originalLine.match(/^\s{2,}/) && !line.match(/^([-*+]|\d+[.)])\s+/)) {
            const lastItem = currentList.items[currentList.items.length - 1];
            lastItem.push(line.trimStart());
            continue;
        }
