...
`,pt=U($(k("State","state"),L("Available Via: elegance-js/server/state"),i("State is, simply put, a collection of variables.",br(),"You initialize it on the server using the ",l("createState()")," function."),h(ae),m(),d("Usage"),i("The ",l("createState()")," function takes in two values.",br(),"The initial value of the state, and an options object.",br(),"The options object may currently only define a bind to the state (more on this later)",br(),br(),"The function stores the created state in the servers current state store,",br(),"so that upon completion of compilation, it may be serialized into page_data."),m(),d("Return Value"),i("The return value of ",l("createState()")," is a State ",y({href:"/docs/concepts#object-attributes",class:"border-b-2"},"Object Attribute, "),br(),"which you can use to refer back to the created state."),h(re),h(se),i("Many functions like load hooks, event listeners, and observe, take in optional SOAs."),m(),d("Bind"),i("State, in the browser, is kept in the global ",l("pd")," object, and indexed via pathnames.",br(),"All state for the pathname ",l("/recipes/apple-pie")," will be in ",l('pd["/recipes/apple-pie"]'),br(),br(),"However, in some scenarios you may want to reference the same state on multiple pages. ",br(),"For this, you may ",b("bind "),"the state to a ",y({href:"/docs/page-files#layouts",class:"border-b-2"},"Layout."),br(),br(),"Then, the state will go into ",l("pd[layout_id]"),", instead of the pathname of the page."),h(ie),m(),d("Important Considerations"),i("State persists it's value during page navigation.",br(),"Meaning if you want it to reset it's value, you must do so yourself."),C(),k("Load Hooks","load-hooks"),L("Available Via: elegance-js/server/loadHook"),br(),br(),d("Basic Usage"),i("Load hooks are functions that are called on the initial page load, and subsequent navigations.",br(),"A load hook is registered using the ",l("createLoadHook()")," function."),h(te),m(),d("Cleanup Function"),i("The return value of a load hook is referred to as a cleanup function.",br(),"It is called whenever the load hook goes out of scope.",br(),br(),"You'll want to do things like ",l("clearInterval() & element.removeEventListener()"),br()," here, so you don't get any unintended/undefined behavior."),h(ne),m(),d("Load Hook Scope"),i("The scope of a load hook is either the page it is on, or the layout it is bound to.",br(),"If a load hook is bound to layout, it is called when that layout first appears.",br(),"Subsequently, its cleanup function will get called once it's bound layout no longer exists on the page.",br(),br(),"To bind a load hook to a layout, use the ",l("bind")," attribute, and pass in a ",y({href:"/docs/page-files#layouts",class:"border-b-2"},"Layout ID"),h(oe)),m(),d("Important Considerations"),i("It's important to note that the load hook function body exists in ",br(),b("browser land ")," not server land. Therefore the code is ",b("untrusted.")),C(),k("Event Listener","event-listeners"),L("Available Via: elegance-js/server/createState"),br(),br(),d("Basic Usage"),i("Event listeners are a type of state, that you can create with the",br(),l("createEventListener()")," function."),h(le),i("This function returns an SOA, which can then be put on any event listener option of an element.",br(),br(),"The eventListener parameter of ",l("createEventListener()")," takes in two types values.",br(),"First, a params object, which by default contains the native event which was triggered."),m(),d("Dependencies"),i("The second parameter, is a spread parameter, containing the dependencies of the event listener."),h(ce),m(),d("Extra Params"),i("You may also extend the params object parameter of the event listener,",br(),"With the ",l("params")," attribute.",br(),br(),"This is handy for when you need to pass some value to the client, ",br(),"that is not necessarily a state variable, but it can change per compilation."),h(pe),m(),d("Important Considerations"),i("It's important to note that the event listener function body exists in ",br(),b("browser land ")," not server land. Therefore the code is ",b("untrusted.")),C(),k("Layouts","layouts"),L("Available Via: elegance-js/server/layout"),br(),br(),i("A layout is a section of a page that is not re-rendered between",br(),"page navigations, to pages that share the same layout order.",br(),br(),"Instead, the layouts ",b("children")," are replaced.",br(),br(),"This has a few advantages. The main one being, that since the elements themselves,",br(),"are not re-rendered, they maintain things like their hover state."),m(),d("Basic Usage"),i("Layouts work a bit differently in Elegance than you may perhaps be used to.",br(),"For example, in Next.JS, layouts are ",b("inherited "),"to every subsequent page.",br(),br(),"So a layout defined at ",l("/")," would apply to ",b("every")," single page.",br(),"Which you may think is nice and saves time, but almost always I find myself in a situation",br(),"where I want a layout for every page of a given depth, except one.",br(),br(),"And then, I have to either move the special page one depth upward",br(),"or the others one-depth downward.",br(),br(),"Conversly, layouts in Elegance are ",b("not "),"inherited, and are are ",b("opt-in."),br(),br(),"To create a layout, use the ",l("createLayout()")," function, and pass in a name.",br(),"The name is used so any subsequent calls to this function by other pages will get the same ID."),h(ue),i("This layout ID can then be passed to state, load hooks, etc."),m(),d("Breakpoints"),i("Creating the actual layout element is simple.",br(),"Just make a function that takes in child elements, and have it return some kind of simple layout."),h(de),i("Then, wrap the children with the built-in ",l("Breakpoint()")," element."),h(be),m(),d("Important Considerations"),i("The ",l("Breakpoint()")," element is the one that gets replaced",br(),"when navigating within any given layout.",br(),br(),"All sibling and parent elements stay untouched.",br(),br(),"Also note, that in complex pages where there are multiple nested layouts,",br(),"the one that has its children replaced, is the layout that is ",b("last shared."),br(),br(),b("For example:"),br(),"Page 1 Layouts: A,B,C,D,E",br(),"Page 2 Layouts: A,B,D,C,E",br(),"In this instance, the ",b("B")," layout is the last shared layout.")));export{pt as page};
