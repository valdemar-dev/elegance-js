...
## Cleanup
Due to the nature of [client-side navigation](https://elegance.js.org/client-side-navigation), the JavaScript context between pages remains the same. This has many benefits, but also at least one drawback. Code that runs every time a page loads, (aka a load hook), could retain some information in memory, which the browser keeps alive. 
...
## Specifics
Load hooks registered in [pages](https://elegance.js.org/pages) are called whenever the user navigates to the page.

Whenever a load hook is registered to a [layout](https://elegance.js.org/layouts), however, it's behavior slightly changes.
