...
`page` should return some kind of [element](https://elegance.js.org/elements), which will become the content of the page; and `metadata` should return an *array* of elements, things like `title`, `link`, etc, which become the *metadata* of the page.

## Parameters
The `page` constructor function gets as it's parameters whatever [layout props](https://elegance.js.org/layouts#layout-props) it's parent layouts gave it, and the [dynamic route params](https://elegance.js.org/dynamic-routes), if the page exists within a dynamic route. 
## Dynamic Pages
...
If you wish to use things like the [router](https://elegance.js.org/router), or respond with dynamic content on each request, it may be desirable to turn the page into a *dynamic page*.
...
This is why the [Elegance Server](https://elegance.js.org/elegance-server) supports *status code pages*.
