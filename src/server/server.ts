...
async function collectMiddlewares(dirs: string[]): Promise < Middleware[] > {
    ...
    for (const dir of dirs) {
        ...
        for (const key of mwKeys) {
            ...
            if (typeof f === 'function' && !middlewares.some(existing => existing === f)) {
                middlewares.push(f as Middleware);
            }
        }
    }
...
async function sendResponse(
    ...
    let bufferBody: Buffer = typeof body === 'string' ? Buffer.from(body) : body;
