...
type EffectCallback<D extends readonly ServerSubject<unknown>[]> =
    (...dependencies: { [K in keyof D]: ClientSubject<D[K]["value"]> }) => any;

class Effect<const T extends readonly ServerSubject<unknown>[]> {
 ...
 */
function effect<const T extends readonly ServerSubject<unknown>[]>(
    callback: EffectCallback<T>,
    dependencies: [...T]
) {
    ...
    const effect = new Effect<T>(callback, dependencies, id);
